...
/-!
## Finite Index Set Bound
-/

/-- The chaining increment bound: for a sub-Gaussian process, the expected
    maximum increment over a finite net is bounded by √log(size of net). -/
theorem chaining_increment_bound {μ : Measure Ω} [IsProbabilityMeasure μ]
    {X : A → Ω → ℝ} {σ : ℝ} (hσ : 0 < σ)
    (hX : IsSubGaussianProcess μ X σ)
    (T : Finset A) (hT : 2 ≤ T.card)
    (t₀ : A) (ht₀ : t₀ ∈ T) (hX_meas : ∀ t, Measurable (X t))
  ...
  obtain ⟨C, hC_pos, _, hbound⟩ := subGaussian_finite_max_bound' hσ hX T hT D hD hdiam hX_meas
    hX_int hX_int_exp
  exact ⟨C, hC_pos, hbound t₀ ht₀⟩

/-- Dudley's bound for finite index sets (covering number is exact cardinality). -/
theorem dudley_finite_bound {μ : Measure Ω} [IsProbabilityMeasure μ]
    {X : A → Ω → ℝ} {σ : ℝ} (hσ : 0 < σ)
    (hX : IsSubGaussianProcess μ X σ)
    (T : Finset A) (hT : T.Nonempty) (hT_card : 2 ≤ T.card)
    {D : ℝ} (hdiam : Metric.diam (T : Set A) ≤ D)
    ...
    μ[fun ω => ⨆ t ∈ T, X t ω] ≤ σ * D * sqrt (2 * log T.card) := by
  have h := subGaussian_finite_max_bound hσ hX T hT hT_card t₀ ht₀ hcenter hX_meas
    hX_int hX_int_exp hdiam_pos
  calc μ[fun ω => ⨆ t ∈ T, X t ω]
    _ ≤ σ * Metric.diam (T : Set A) * sqrt (2 * log T.card) := h
...
/-- The dense sequence from a totally bounded set is indeed dense. -/
lemma denseRange_denseSeqInTB {s : Set A} (hs : TotallyBounded s) (hsne : s.Nonempty) :
    DenseRange (denseSeqInTB hs hsne) := by
  letI : Nonempty (↥s) := hsne.to_subtype
  letI : SeparableSpace (↥s) := hs.isSeparable.separableSpace
  unfold denseSeqInTB
  exact denseRange_denseSeq (↥s)
