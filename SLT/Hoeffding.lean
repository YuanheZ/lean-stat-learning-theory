import SLT.ForMathlib.Probability.Moments
  ...
  have q : ∀ x : ℝ, ∃ c ∈ (Set.Ioo 0 t), f t = f 0 + f' 0 * t + f'' c * t ^ 2 / 2 := by
    ...
    have q0 : f t = f 0 + f' 0 * t + A * t ^ 2 / 2 := by
      have q0' : A * t ^ 2 = (f t - f 0 - f' 0 * t) * 2 := by
        ...
        _ = (f t - f 0 - f' 0 * t) * 2 := by field_simp
      rw [q0']
      ring
    ...
    have q1 : g 0 = 0 := by
      dsimp only [g, A]
      calc
      _ = f t - f 0 - f' 0 * t - (f t - f 0 - f' 0 * t) * 2 / 2 * t ^ 2 / t ^ 2 := by field_simp; ring
    ...
    have q3 : ∀ x : ℝ, HasDerivAt g (g' x) x := by
      ...
      · rw [← (by ring : 0 - f' x + (f' x - f'' x * (t - x)) = - f'' x * (t - x))]
        ...
        · ext; simp only [Pi.mul_apply]; ring
        · dsimp [f', f'']
...
theorem hoeffding_nonneg [IsProbabilityMeasure μ]
  ...
  dsimp [mgf]
  by_cases w : t = 0;
    · rw [w]; simp only [zero_mul, exp_zero, integral_const, probReal_univ,
    smul_eq_mul, mul_one, ne_eq, OfNat.ofNat_ne_zero, not_false_eq_true,
