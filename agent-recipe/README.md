...
We provide a structured approach to leveraging AI agents for Lean 4 proof formalization. The workflow is designed to maximize agent efficiency, minimize context window usage, and produce clean, warning-free code.
...
- **Leverage MCP tools**: Use Lean LSP and search tools for efficient declaration discovery
...
Break down your proof into lemmas as small as possible. The goal is for the agent to complete each lemma's formalization within a single context window **without** auto-compaction.
...
- Write only the signatures of needed declarations in your instructions
...
```
Please modify the formalization code to eliminate the warning messages. **IMPORTANT:** For unused variables, you need to directly remove them instead of adding `_`, then correct the downstream call of modified results in @SLT/ . Please use MCP tools to diagnose. The target file to modify is [FILENAME]
```
